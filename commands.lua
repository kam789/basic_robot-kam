...
basic_robot.plant_table = {
    ...
    ["ethereal:wild_onion_plant"] = "ethereal:onion_1",
...
local check_operations = function(name, amount, quit)
    if basic_robot.maxoperations ~= 0 then
        ...
        if operations >= 0 then
            ...
            if quit then
                error(
                    "Robot out of available operations in one step (1s). View available operations with self.operations() and check help to see how much operations each action requires."); return false
            end
            return false
        end
    end
    ...
    minetest.register_node("basic_robot:button" .. number,
        {
            ...
            is_ground_content = false,
            groups = { cracky = 3, not_in_craft_guide = 1, not_in_creative_inventory = 1 },
            on_punch = function(pos, node, player) button_punched(pos, node, player, type) end
        })
end
...
basic_robot.commands.craft = function(item, mode, idx, amount, name)
    ...
    if not cache then
        basic_robot.commands.craftcache[name] = {}; cache = basic_robot.commands.craftcache[name]
    end
            ...
            if k then
                ...
                for i = 1, size do
                    ...
                    if groups[group] then
                        cache.itemlist[item] = nil; cache.itemlist[itemname] = v
                        break
                    end
                end
            end
    ...
    for i = 1, string.len(input) do
        ...
        if i % 8 == 1 then                                                          -- every 8 characters new offset using strong hash function incorporating recent offsets in nonlinear way
            block_offset = get_hash(
                _G.minetest.get_password_hash("", i * (offset + 1) .. password .. (block_offset ^ 2)),
                n);                                                                                             -- composite fun with more serious hash function
...
local cmd_get_player = function(data, pname) -- return player for further manipulation
    ...
    if not player then
        error("player does not exist"); return
    end
    local spos = data.spawnpos;
...
local cmd_set_triggers = function(pdata, triggers)
    ...
    for i = 1, #triggers do
        ...
        if not tdata then
            triggerdata[id] = {}; tdata = triggerdata[id]
        end
...
basic_robot.commands.puzzle = {
    ...
    get_meta = function(data, pos)
        ...
        local meta = minetest.get_meta(pos);
        if not meta then
            error("get_meta in puzzle returned nil"); return
        end
        return meta
    end,
