...
function getSandboxEnv(name)
    ...
    local env =
    {
        ...
        boost = function(v)
            ...
            if v == 0 then
                local pos = obj:get_pos(); pos.x = math.floor(pos.x + 0.5); pos.y = math.floor(pos.y + 0.5); pos.z = math
                    .floor(pos.z + 0.5);
        ...
        self = {
            ...
            spawnpos = function()
                local pos = basic_robot.data[name].spawnpos; return { x = pos.x, y = pos.y, z = pos.z }
            end,
            name = function() return name end,
            operations = function() return basic_robot.data[name].operations end,
            viewdir = function()
                local yaw = basic_robot.data[name].obj:getyaw(); return { x = math.cos(yaw), y = 0, z = math.sin(yaw) }
            end,
    ...
    for dir, dir_id in pairs(directions) do
        env.check_inventory[dir] = function(itemname, inventory, i)
            return commands.check_inventory(name, itemname,
                inventory, i, dir_id)
        end
    end
    env.check_inventory.self = function(itemname, inventory, i)
        return commands.check_inventory(name, itemname, inventory,
            i, 0)
    end;
    ...
    for dir, dir_id in pairs(directions) do
        env_machine_place_seed[dir] = function(seedbookname)
            return commands.machine.place_seed(name, dir_id,
                seedbookname)
        end
...
local spawn_robot = function(pos, node, ttl)
    ...
    if id <= 0 then               -- just compile code and run it, no robot entity spawn
        ...
        if data.lastpos ~= hashpos then
            data.lastpos = hashpos; poschange = true
        end
    ...
    if fields.OK then
        ...
        if fields.code then
            ...
            if string.len(code) > 64000 then
                minetest.chat_send_all("#ROBOT: " .. name .. " is spamming with long text.")
                return
            end
    ...
    function(player, formname, fields)
        ...
        if string.find(formname, robot_formname) then
            ...
            elseif fields.dig then
                pcall(function()
                    basic_robot.data[name].operations = basic_robot.maxoperations; commands.dig(name, 3)
                end)
            ...
            elseif fields.digdown then
                pcall(function()
                    basic_robot.data[name].operations = basic_robot.maxoperations; commands.dig(name, 6)
                end)
            elseif fields.digup then
                pcall(function()
                    basic_robot.data[name].operations = basic_robot.maxoperations; commands.dig(name, 5)
                end)
            end
            return
        end
        ...
        if string.find(formname, robot_formname) then
            ...
            if fields.listname then
                ...
                if string.sub(list, 1, 3) == "CHG" then
                    ...
                    minetest.show_formspec(name, "robot_editor_:" .. minetest.pos_to_string(pos),
                        code_edit_form(pos, name));
                end
...
minetest.register_on_chat_message(
    function(name, message)
        ...
        if string.sub(message, 1, 1) == "\\" then
            hidden = true; message = string.sub(message, 2)
        end
...
minetest.register_node("basic_robot:spawner", {
    ...
    after_place_node = function(pos, placer)
        ...
        if authlevel == -1 then
            ...
            minetest.chat_send_player(owner, "Unable to use robot, invalid privs!")
        end
...
minetest.register_privilege("bot", "allow player to use robots")
